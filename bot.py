...
# CF registration opens roughly 5 days before contest start
REG_WINDOW_SECS = 5 * 24 * 3600
...
def reg_status(ts):
    """
    If more than 5 days until start  → registration not open yet
    If within 5 days                 → registration open
    If past start                    → closed
    """
    now  = datetime.now(timezone.utc)
    diff = (datetime.fromtimestamp(ts, tz=timezone.utc) - now).total_seconds()
    if diff > REG_WINDOW_SECS:
        reg_open_ts = int(ts - REG_WINDOW_SECS)
        return f"Before Registration  ·  opens <t:{reg_open_ts}:R>"
    if diff > 0:
...
def embed_upcoming_list(contests):
    """
    contests: sorted ascending by startTimeSeconds (soonest first).

    Display order in embed:  furthest (#N) at top  →  soonest (#1) at bottom.
    Numbering:  #1 = soonest,  #N = furthest.
    """
    if not contests:
        e = discord.Embed(
    ...
    # Reverse so furthest (#N) is at top, soonest (#1) is at bottom
    blocks.reverse()
...
def embed_announce(c, num_in_list):
    ...
    e.add_field(name="🏷️  Division",     value=label,             inline=True)
    e.add_field(name="⏳  Countdown",    value=f"<t:{ts}:R>",     inline=True)
    e.add_field(name="\u200b",           value="\u200b",           inline=True)
    e.add_field(name="📅  Start Time",   value=fmt_bd(ts),        inline=False)
...
async def task_announce():
    ...
    # Pre-seed: mark all currently-known contests as announced (avoid restart spam)
    try:
        all_c = await fetch_cf()
        for c in all_c:
